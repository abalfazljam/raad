...
> **جدید در نسخه 1.7:** تجربه کامل IDM — دانلود از مرورگر خودکار و بلافاصله شروع می‌شود، پنجره شناور پیشرفت باز می‌شود و بعد از پایان، کارت «کامل شد» با اجرای فایل، باز کردن پوشه، کپی فایل و درگ‌اند‌دراپ. اجرای خودکار با ویندوز (کنار ساعت)، افزونه فارسی‌محور، و رفع مشکل از دست رفتن دانلودهای رهگیری‌شده. جزئیات در [تغییرات نسخه 1.7](#تغییرات-نسخه-17).
...
- [تغییرات نسخه 1.7](#تغییرات-نسخه-17)
...
1. فایل **`Raad-Setup-1.7.0.exe`** را دانلود و اجرا کن.
...
1. فایل **`RaadDM-Portable-1.7.0.exe`** را دانلود کن.
...
تمرکز 1.7: **تجربه کاربری IDM-مانند + قابلیت اطمینان دانلود.**

| تغییر | توضیح |
|---|---|
| 🛡 رفع «وصل است ولی دانلود نمی‌شود» | در 1.6 افزونه اول دانلود مرورگر را لغو می‌کرد و بعد به برنامه می‌فرستاد؛ اگر ارسال شکست می‌خورد، دانلود از دست می‌رفت. حالا **اول ارسال، بعد لغو** — اگر برنامه نپذیرد، دانلود عادی مرورگر ادامه می‌یابد. دانلودهای تمام‌شده هم دست‌نخورده می‌مانند (بدون دانلود تکراری) |
| 🍪 کوکی مرورگر پیش‌فرض روشن | مثل IDM، سشن لاگین مرورگر همراه هر دانلود ارسال می‌شود — لینک‌های نیازمند ورود بدون خطا |
| ⚡ شروع خودکار دانلود افزونه | کلیک دانلود در مرورگر = شروع فوری در رعد + پنجره پیشرفت (بدون پرسش؛ قابل خاموش‌کردن از تنظیمات) |
| 📊 پنجره شناور پیشرفت | نوار پیشرفت زنده، سرعت، زمان باقی‌مانده و دکمه‌های توقف/ادامه/لغو — پایین‌راست صفحه |
| 🏁 کارت «کامل شد» | اجرای فایل، نمایش در پوشه، کپی فایل (Ctrl+V در Explorer) و **درگ‌اند‌دراپ** فایل به هر پوشه‌ای |
| 🖥 اجرای خودکار با ویندوز | پیش‌فرض روشن؛ برنامه کنار ساعت (تری) باز می‌شود و دانلودهای مرورگر را همان‌جا می‌گیرد |
| 🇮🇷 افزونه فارسی‌محور | پاپ‌آپ و منوی راست‌کلیک به‌پیش‌فرض فارسی؛ با دکمه EN انگلیسی می‌شود |
| 🖼 آیکن واقعی همه‌جا | اینستالر/پرتابل/برنامه در تسک‌بار و تسک‌منیجر با آیکن رعد — نه آیکن الکترون |
...
- ✅ **v1.7:** جریان کامل IDM (شروع خودکار + پنجره پیشرفت + کارت کامل‌شد با درگ)، اجرای خودکار کنار ساعت، افزونه فارسی‌محور
