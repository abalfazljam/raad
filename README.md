...
- 🔄 **انتقال از IDM** — «کل لیست داخل خود IDM» از رجیستری ویندوز + همه فایل‌های `UrlHistory*` (ادغام و حذف تکراری، تا ۲۰٬۰۰۰ مورد). حتی اگر فایل‌ها سال‌هاست پاک شده باشند همه می‌آیند؛ موارد به‌صورت «آماده» وارد می‌شوند و **هیچ‌کدام خودکار دانلود نمی‌شود** — فقط هر کدام را خودت شروع کنی
...
لیست اصلی IDM (همان چیزی که در پنجره‌اش می‌بینی) داخل **رجیستری ویندوز** ذخیره می‌شود و `UrlHistory.txt` هم آینه‌ی فعالیت‌های اخیر است. «انتقال خودکار» رعد **هر دو منبع** را می‌خواند و یکی می‌کند — فرقی نمی‌کند فایل‌ها پاک شده باشند؛ همه می‌آیند و هیچ‌کدام خودکار دانلود نمی‌شود:

| روش | توضیح |
|---|---|
| **خودکار** (همان ویندوز) | در برنامه: «انتقال از IDM ← انتقال خودکار» — کل لیست از رجیستری (`HKEY_CURRENT_USER\Software\DownloadManager`) + همه فایل‌های `UrlHistory*` در `%APPDATA%\IDM`. فقط IDM باید بسته باشد. |
